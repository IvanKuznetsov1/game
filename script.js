const Number1_19 = ['', 'один', 'два', 'три', 'четыре', 'пять', 'шесть', 'семь', 'восемь', 'девять', 'десять', 
                        'одиннадцать', 'двенадцать', 'тринадцать', 'четырнадцать', 'пятнадцать', 'шестнадцать', 'семнадцать', 
                        'восемнадцать', 'девятнадцать'];

const Number20_90 = ['', '', 'двадцать', 'тридцать', 'сорок', 'пятьдесят', 'шестьдесят', 'семьдесят',  
                        'восемьдесят', 'девяносто'];

const Number100_900 = ['', 'сто', 'двести', 'триста', 'четыреста', 'пятьсот', 'шестьсот',  
...
function textNumber() {
    ...
    if (textCount < 20) {
        textOfNumber = arrayNumber1_19[textCount];
            ...
            textOfNumber = Number20_90[textCount] + " " + Number1_19[ostatok1];
            }
            else if (textCount < 1000) {
                ostatok1 = textCount%100;
                if(ostatok1 < 20) {
                    textCount = parseInt(textCount/100);
                    textOfNumber = Number100_900[textCount] + " " + Number1_19[ostatok1];
                }
                else {
                    let ostatok2 = ostatok1%10;
                    ostatok1 = parseInt(ostatok1/10);
                    textCount = parseInt(textCount/100);
                    textOfNumber = Number100_900[textCount] + " " + Number20_90[ostatok1] + " " + Number1_19[ostatok2];
                }    
        }
    ...
    if (gameRun){
        ...
        switch (phraseRandom3){
            case 0:
                ...
                break;
            case 1:
                answerField.innerText = `Ха-ха, опять угадал!\n\u{1F601}`;
                break;
            case 2:
